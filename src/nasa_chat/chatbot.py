...
class ChatBot:
    ...
    def ask(self, query: str) -> Dict[str, Any]:
        ...
        - Search NASA Image Library with the query
        - Use the first result's description as technical context
        - Simplify in English, then translate if target language is Arabic
        """
        results = self.nasa.search_images(query)
        if not results:
        ...
        simple_en = self.simplifier.simplify(technical_desc, audience=self.audience)
        simple = (
            self.translator.translate(simple_en, source_lang="en", target_lang="ar")
            if self.language == "ar"
            else simple_en
        ...
        resp = {"technical": technical_desc, "simple": simple, "sources": formatted_sources}
        self._remember(query, resp)
        return resp
